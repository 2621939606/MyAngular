...
export class TextareaLimiterDirective {
  ...
  private valueShowProcess(str: string = this.userInputValue): string {
    ...
    let newLineNum:number = this.userInputValue.split(this.newLineMarker).length - str.split(this.newLineMarker).length;

    this.cursorLocalStart = 
  ...
  private async setCursorValueAndLocal(
    ev: Event,
    ...
    await this.thisControl.setValue(this.userInputValue.replaceAll(this.newLineMarker, '\n'));
